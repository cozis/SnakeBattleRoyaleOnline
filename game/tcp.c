...
TCPHandle tcp_server_create(string addr, uint16_t port, int max_clients)
{
    ...
    if (addr.count == 0)
        ...
        char *addr_copy = alloc(get_temporary_allocator(), addr.count+1);
        memcpy(addr_copy, addr.data, addr.count);
        addr_copy[addr.count] = '\0';

        if (inet_pton(AF_INET, addr_copy, &addr2) != 1) {
            close(fd);
            return TCP_INVALID;
        }
    }
    ...
    void *mem = alloc(get_heap_allocator(), max_clients * sizeof(TCPClient) + MAX_EVENTS_PER_CLIENT * max_clients * sizeof(TCPEvent));
    if (!mem) {
        close(fd);
        return TCP_INVALID;
    }
...
void tcp_server_delete(TCPHandle handle)
{
    ...
    close(server->fd);
    dealloc(get_heap_allocator(), server->clients);
...
/*
static void print_bytes(const char *prefix, char *ptr, size_t num)
{
...
static int move_bytes_from_socket_to_queue(int fd, ByteQueue *queue, size_t *moved)
{
    ...
    for (;;) {
        ...
        //print_bytes("i | ", dst, (size_t) n);

        byte_queue_end_write(queue, (size_t) n);
    }
    size_t moved_ = byte_queue_used_space(queue) - start;
    if (moved) *moved = moved_;
    return !disconnect;
}
...
string tcp_client_get_input(TCPHandle handle)
{
    TCPServer *server;
    TCPClient *client = client_from_handle(handle, &server);
    if (client == NULL) abort();
...
TCPHandle tcp_client_create(string addr, uint16_t port)
{
    ...
    if (addr.count == 0)
        ...
        if (inet_pton(AF_INET, addr_copy, &parsed_addr) != 1) {
            close(fd);
            return TCP_INVALID;
        }
    }
